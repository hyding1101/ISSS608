...
[**ggstatsplot**](https://indrajeetpatil.github.io/ggstatsplot/index.html) ![](https://r4va.netlify.app/chap10/img/image1.jpg){width="19"} is an extension of [**ggplot2**](https://ggplot2.tidyverse.org/) package for creating graphics with details from statistical tests included in the information-rich plots themselves.

```         
-   To provide alternative statistical inference methods by default.
-   To follow best practices for statistical reporting. For all statis
```
...
|------------------|------------------------------------------------------|
| type = "bayes" | **Performs Bayesian statistical analysis** instead of frequentist tests. |
| test.value = 60 | **Tests whether the mean English score is significantly different from 60**. |
:::
...
::: callout-note
`"np"` stands for **"non-parametric"**, meaning it uses a test that **does not assume a normal distribution** of the data.

messages = FALSE-**Prevents unnecessary messages or warnings from being printed** while running the function.
:::
...
| Parameter | Description |
|-------------------|-----------------------------------------------------|
| mean.ci = TRUE | Indicates that the plot will display the mean and the confidence interval (CI) for each group. |
| pairwise.comparisons = TRUE | Enables pairwise comparisons to examine the differences between groups. |
| p.adjust.method = "fdr" | The method for adjusting p-values, using the False Discovery Rate (FDR) to correct for multiple comparisons. |
:::
...
::: callout-note
-   `marginal = TRUE`: Displays marginal plots, which show the distribution of each variable (in this case, `MATHS` and `ENGLISH`) on the sides of the scatter plot (typically histograms or density plots).

-   `marginal = FALSE`: Disables the marginal plots, meaning only the scatter plot will be shown without the additional distribution plots on the margins.
:::
...
::: callout-note
This code creates a linear regression model with the goal of predicting `Price` (car price). Here, `Price` is the dependent variable (the variable being predicted), while `Age_08_04` (car age), `Mfg_Year` (manufacturing year), `KM` (kilometers driven), `Weight` (car weight), and `Guarantee_Period` (warranty period) are the independent variables (the predictors).
:::
...
#remotes::linstall_github("easystats/see")
library("bayestestR")
library("see")
```
