...
\*The syntax description of [*pairs*](https://stat.ethz.ch/R-manual/R-devel/library/graphics/html/pairs.html)function.
...
-   Defines the function `panel.cor`, which takes `x` and `y` as input, representing two variables.

-   `digits=2`: The correlation coefficient is rounded to 2 decimal places.

-   `prefix=""`: No prefix is added by default.

-   `par("usr")` stores the current plot coordinate system.

-   `on.exit(par(usr))` ensures that the original coordinate system is restored when the function ends.

-   `par(usr = c(0, 1, 0, 1))` sets the **plot area to a normalized scale (0 to 1)**, ensuring the text is positioned correctly.

-   `use="complete.obs"`: Ignores missing values (NA)

-   `cex = cex.cor * (1 + r) / 2`: The **higher the correlation, the larger the font size**, making it visually more intuitive.
:::
...
Reference: [An Introduction to corrplot Package](https://cran.r-project.org/web/packages/corrplot/vignettes/corrplot-intro.html)
...
::: callout-note
## Other method Options in corrplot()
...
|  |  |
|----|----|
| `"circle"` | Default. Uses **colored circles** (bigger = stronger correlation). |

|            |                                               |
|------------|-----------------------------------------------|
| `"square"` | Uses **colored squares** to show correlation. |

|             |                                     |
|-------------|-------------------------------------|
| `"ellipse"` | Uses **ellipses** (like PCA plots). |

|            |                                                   |
|------------|---------------------------------------------------|
| `"number"` | Displays **correlation coefficients** as numbers. |

|           |                                         |
|-----------|-----------------------------------------|
| `"shade"` | Uses **shaded bars** (gradient effect). |

|           |                                          |
|-----------|------------------------------------------|
| `"color"` | Uses **colored tiles** (like a heatmap). |

|         |                                                       |
|---------|-------------------------------------------------------|
| `"pie"` | Uses **pie chart segments** to represent correlation. |
:::
...
::: callout-note
| Parameter | Explanaton                   |
|-----------|------------------------------|
| tl.pos    | Position of variable labels  |
| tl.cex    | Font size of variable labels |
| tl.offset | Offset of variable labels    |
| cl.pos    | Position of color legend     |
| cl.cex    | Font size of color legend    |
| cl.offset | Offset of color legend       |
:::
...
| Code                | Explanation                                  |
|---------------------|----------------------------------------------|
| p.mat = wine.sig\$p | Matrix of p-values for significance testing  |
| sig.level = 0.05    | Only show correlations where p-value \< 0.05 |
:::
...
The `order` parameter controls the arrangement of variables (rows and columns), affecting the visualization of the correlation matrix.

**Common Options:**

-   **`"original"`** (default): Arranges variables in their original order.

-   **`"AOE"`** (Angular Order of the Eigenvectors): Sorts variables based on **Eigen Decomposition** of the correlation matrix, grouping highly correlated variables together to form a clearer structure.

-   **`"FPC"`** (First Principal Component): Orders variables based on **Principal Component Analysis (PCA)**, placing the most important variables together.

-   **`"hclust"`** (Hierarchical Clustering): Sorts variables using **Hierarchical Clustering**, similar to a dendrogram.

    -   You can specify different clustering methods using `hclust.method = "ward.D"`.
:::
