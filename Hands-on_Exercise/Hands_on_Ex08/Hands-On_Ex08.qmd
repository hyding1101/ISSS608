...
`mutate_if()` is a function from the `dplyr` package that applies a transformation **only to specific variable types** based on a given condition.

`is.character`

-   `is.character` is a logical function used to check whether a variable is of **character type (string)**.

`as.factor`

-   `as.factor()` is used to convert a variable into a **factor (categorical variable)**.
:::
...
-   `seq(1, max(df$Positive), 1)`: Creates a sequence ranging from 1 to the maximum value in the `df$Positive` column, increasing by 1 at each step.

-   `number.seq` represents different sample sizes and is used to assess how varying sample sizes impact the confidence interval calculations.
:::
...
-   **`annotate("text", ...)`**: Adds a text annotation to the plot.

-   **`x = 1, y = -0.13`**: Specifies the position of the annotation, where `x = 1` represents the first position and `y = -0.13` (possibly because the y-axis includes negative values in certain cases).

-   **`label = "95%"`**: The text content of the annotation is `"95%"`, indicating the 95% confidence interval.

-   **`size = 3`**: Sets the font size to 3.

-   **`colour = "grey40"`**: Sets the text color to dark gray (`grey40`).
:::
