...
-   **Key parameter explanations**:
    ...
    -   `by = "OUTLET TYPE"`
        → **Divides the map into separate facets based on the `OUTLET TYPE` variable.**

    -   `nrow = 1`
        → **Arranges the facets in a single row (horizontally).**

    -   `sync = TRUE`
        → **Ensures that all facets share the same zoom level and map extent.**
:::
